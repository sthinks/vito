...
function MainProjects() {
    ...
    const getAllMainProjects = async () => {
        ...
        if (param.slug === "main-projects") {
            const filterData = result.data.filter((x) => x.continues === 0);
            setProjects(filterData);
        } else {
            const filterData = result.data.filter((x) => x.continues === 1);
            setProjects(filterData);
        }
    };
    if (inView === true) {
    ...
    return (
        <>
            <div className="w-full flex justify-center items-center bg-blue-400 pb-32 pt-56">
                <p className="text-6xl font-extrabold tracking-widest text-white">
                    {param.slug === "main-projects"
                        ? "MAIN PROJECTS"
                        : "ONGOING PROJECTS"}
                </p>
            </div>
                        ...
                        <div className="w-full flex flex-wrap justify-evenly px-16 max-xl:px-5 max-md:px-2 max-md:justify-between max-sm:flex-col max-sm:justify-center max-sm:items-center">
                            {projects?.map((item, i) => (
                                <div
                                    ...
                                    className={
                                        projectsAnim
                                            ? "w-[30%] my-2 h-3/4 max-md:w-[30%] max-md:h-[60%] max-sm:w-5/6 max-sm:my-2 bg-white relative shadow-lg animate-card-first"
                                            : "w-[30%] my-2 h-3/4 max-md:w-[30%] max-md:h-[60%] max-sm:w-5/6 max-sm:my-2 bg-white relative shadow-lg animate-card-down-first"
                                    }
                                >
